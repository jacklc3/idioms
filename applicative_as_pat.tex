...
We start with the free applicative view \cite{capriotti-free-applicatives}, where we see a computation $u: J(X)$ to be a list of effects paired with a pure function consuming the return types of those effects:
\begin{equation}
    \inferrule{(\Gamma \vdash u_i : J(X_i))_{i=1...k} \and \Gamma \vdash f : \prod_i X_i \to X}{\Gamma \vdash ([u_1...u_k], f): J(X)}
\end{equation}
This formulation comes from the `idiomatic brackets' normal form coined by McBride and Patterson \cite{mcbride-applicatives}, and directly follows from the idea of representing an applicative as 
\begin{quote}
``The sum for all effect sequences with a map that processes each effect's return value"
\end{quote}
that @Sean presented last meeting. Say we only allow a certain class of finitary ground effects $e_i: J(n_i)$, we can get 
\begin{equation}
...
\begin{question}[Naive]
  Do we lose expressiveness by only allowing these `ground' effects? In fact do we even still get an applicative? 
\end{question}
...
My intuition is that an applicative operation $e: J(n)$ should not correspond to just $J(X^n) \to J(X)$, but rather a transformation $$e_m: J((X^{m})^n)\to J(X^{m})$$ natural in $m$, where we allow this extra $m$ to be added in a natural way.
...
\]
where we think of $C$, the carrier presheaf, as $C(n) = J(X^n)$. If instead we formulated a multi-sorted theory where the parameter category $P$ contains the free products of these $n$'s, we'd instead interpret the same $n$-ary applicative operation as a natural transformation
\[
...
Let us now formulate my approach in a bit more detail. We essentially treat an applicative effect $J(n)$ directly as a parameter/resource of sort $n$. We consider a discretely parameterised theory with terms generated as follows:
...
\end{mathpar}
Note the difference with the scoped effects paper \cite{lindley-scoped-effects, matache-scoped-effects}: instead of considering the parameters as akin to a stack, we'd like to consider it as some sort of free non-symmetric monoidal category. Why do we not allow symmetry? Because in applicatives, while the effects are associative, they are not commutative, and the parameter context now records the order in which these effects will be executed at the end, much like the list of effects $[e_i:n_i]_i$ in the free applicative form. 


Given an (finitely presented???) applicative $J$, for each of its operations $\idiomop: n$ we can now formulate operations:
\[
  \opnew_{\idiomop}: ([] \mid [n])
\]
as well as more things like maybe an operation for 
\[
...
\begin{question}
  Does this formulation make sense wrt equations between operations in a free applicative? 
\end{question}

Two remarks. Firstly, we can now easily formulate the case where multiple effects are performed:
\[
...
Secondly, because our rules allows one to perform an effect at an arbitrary point in the parameter context, it means that in particular, we can perform an applicative operation $\idiomop: n$ at \emph{any point} in the program. In other words, the order of operations does not matter; all that matters is the order in which they are laid out in the parameter context. This feels like the `right' thing to do for applicatives.

\subsection{Relation to monads?}
One potentially cool feature that this formulation could have is its connection to our intitial note and @Jack's ideas about a dual context calculus, which integrates the Idiom and the Kleisli categories together and has the potential of formalising the applicative do notation. Here, we by construction allow both views to be presented. Indeed, notice how, so far, we have not yet used a context $\Gamma$ with more than one continuation variable; a single continuation variable along with the $\Delta$ context suffice for the applicative view. At the same time, it is not hard to see that the $\Gamma$ context alone allows one to specify non-parameterised algebraic operations! 
