...
\begin{defn}
  An \emph{idiom} is a pointed lax monoidal endofunctor on $\CCat$. That is, an idiom $(W, \unit, \varepsilon, \mult)$ consists of:
  \begin{itemize}
    \item An endofunctor $W: \CCat \to \CCat$,
    \item A family of maps $\unit_A: A \to WA$ natural in $A \in \CCat$,
    \item A map $\eta: 1 \to W1$,
    \item A family of maps $\mult_{A,B}: WA \times WB \to W(A\times B)$ natural in $A,B \in \CCat$,
  \end{itemize}
  such that the following diagrams commute.
  \begin{mathpar}
    \begin{tikzcd}[column sep=large]
      {(WA\times WB)\times WC} & {W(A\times B) \times WC} & {W((A\times B)\times C)} \\
      {WA\times (WB\times WC)} & {WA\times W(B\times C)} & {W(A\times (B\times C))}
      \arrow["{\phi_{A,B}\times WC}", from=1-1, to=1-2]
      \arrow["{\alpha_{WA,WB,WC}}"', from=1-1, to=2-1]
      \arrow["{\phi_{A\times B, C}}", from=1-2, to=1-3]
      \arrow["{W\alpha_{A,B,C}}", from=1-3, to=2-3]
    ...
    \begin{tikzcd}[column sep=large]
      ...
      \arrow["{W\lambda_A}", from=2-2, to=2-1]
    \end{tikzcd}
    \and
    \begin{tikzcd}[column sep=large]
      {A\times B} & {WA\times WB} \\
...
Given an idiom $(W_0, \unit, \mult)$, on a Cartesian closed category $\CCat$, there is a $\CCat$-enriched functor $W : \CCat \to \CCat$ that has same mapping on objects
as $W_0$, and its mapping on hom-objects $W_{A,B}: [A,B] \to [WA, WB]$ is given by the transpose of:
\[
  [A,B] \times WA \xrightarrow{\unit_{[A,B]} \times WA} W[A,B] \times WA \xrightarrow{\mult_{[A,B], A}} W([A,B] \times A) \xrightarrow{W\mathsf{ev}_{A,B}} WB
\]
...
\begin{prop}
  ...
  \[
    \CCat \xrightarrow{L} \Idm W \xrightarrow{R} \CCat
  \]
  Where $\Idm W$ is the change of base of $\CCat$ along $W$, $L$ is an identity on objects functor that acts on hom-objects via $\unit$, and $R$ sends $A$ to $WA$,
  and $W[A,B]$ to $[WA, WB]$ via the lax monoidal structure of $W$.
...
\begin{defn}
  For a CCC $\CCat$ considered self-enriched. We define the category $\Idm W$ as the image of the category $\CCat$ under the change of base along $W$. That is,
  the objects of $\Idm W$ are the same as the objects of $\CCat$, and the morphisms are
  \[
    \Idm W [A, B] = W[A,B].
  \]
\end{defn}
