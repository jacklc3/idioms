...
\begin{lstlisting}
  u ** pure x = pure (\f -> f x) ** u
\end{lstlisting}
in a closed category, because application does not exists. (The application in the other law, \lstinline{(\f -> f ())}, is really just $\iota$ as $f : [I,B]$.)
However, in the setting of a symmetric monoidal closed category, Conor's interchange law can be derived from the simpler closed laws.
