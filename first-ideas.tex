...
\begin{proof}
  ...
  \]
  The equations follow from a diagram chase. \todo{J: I still need to do this for the reverse direction}
\end{proof}
    ...
    \item A monoidal tensorial strength $\theta: ({-} \otimes J{=}) \to J({-}\otimes {=})$. That is, the following diagram commutes.
      \begin{mathpar}
        \begin{tikzcd}[column sep=large]
          ...
          \arrow["{\theta^l_{A,B}\otimes JC}", from=1-1, to=1-2]
          ...
          \arrow["{\theta^l_{A,B\otimes C}}"', from=2-2, to=2-3]
        \end{tikzcd}
      \end{mathpar}
  \end{enumerate}
\end{prop}
