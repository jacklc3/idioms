...
\begin{defn}
  Let $(\CCat, I, [{-}, {=}], \iota, \jmath, \psi)$ be a closed category. An \emph{idiom} is a pointed closed endofunctor on $\CCat$.
  ...
  \begin{mathpar}
    \begin{tikzcd}
      I & {[JA,JA]} \\
      JI & {J[A,A]}
      \arrow["{\jmath_{JA}}", from=1-1, to=1-2]
      \arrow["\varepsilon"', from=1-1, to=2-1]
      \arrow["{J\jmath_A}"', from=2-1, to=2-2]
...
\begin{lstlisting}
  u ** pure () = pure ($ ()) ** u
\end{lstlisting}
It is not possible to express the interchange law of applicative functors
\begin{lstlisting}
  u ** pure x = pure ($ x) ** u
\end{lstlisting}
in a closed category, because application does not exists. (The application in the other law, \lstinline{($ ())}, is really just $\iota$ as $f : [I,B]$.)
However, in the setting of a symmetric monoidal closed category, the interchange law can be derived from the simpler closed laws.
Hence, this perspective has shown us that the applicative functor laws in Haskell are over specified!
...
The following results link this presentation to the above definition. It seems like folklore, but I haven't seen it properly written down anywhere.
    ...
    \item A closed strength $\theta: [{-}, {=}] \to [J{-}, J{=}]$. That is, the following diagrams commute.
      \begin{mathpar}
        ...
        \begin{tikzcd}
          I & {[A,A]} \\
          & {[JA,JA]}
          \arrow["{\jmath_A}", from=1-1, to=1-2]
          \arrow["{\jmath_{JA}}"', from=1-1, to=2-2]
...
We next give a result that connects strong and pointed endofunctors in the monoidal setting.

\begin{prop} \label{prop:strong-pointed-monoidal}
  Let $(\CCat, I, \otimes, \lambda, \rho, \alpha)$ be a symmetric monoidal category and $(J, \varepsilon, \phi): \CCat \to \CCat$ a lax monoidal endofunctor.
...
We now establish the connection between the closed setting and the monoidal setting when the category is symmetric monoidal closed.

\begin{thm} \label{prop:idiom-corresp}
  Let $(\CCat, I, \otimes, [{-},{=}])$ be a symmetric monoidal closed category and $J: \CCat \to \CCat$ an endofunctor.
  Then, the following are equivalent:
  \begin{enumerate}
    \item The data to make $J$ a pointed lax closed functor.
    \item The data to make $J$ a strong lax closed functor.
    \item The data to make $J$ a pointed lax monoidal functor.
    \item The data to make $J$ a strong lax monoidal functor.
  \end{enumerate}
\end{thm}

\begin{proof}
  The correspondence between items (1) and (2) are given by \Cref{prop:strong-pointed-closed}.
  The correspondence between items (3) and (4) are given by \Cref{prop:strong-pointed-monoidal}.
  It is well-known that there is a correspondence between the data to make $J$ a lax monoidal functor and a lax closed functor when $\CCat$ is symmetric monoical closed~\cite{kock-strong-functors}.
  However, we do not know of any proof showing that the data for coherent strengths or pointedness are equivalent too.
  The correspondence between coherent pointedness has fewer proof obligations with only one pair of diagrams required to be equivalent across the closed adjunction.
  This is shown by a straightforward diagram chase.
\end{proof}

This formally aligns the commonly stated folklore that an idiom is \emph{just} a strong lax monoidal endofunctor by closing all sides of the correspondence square.
\[
  \begin{tikzcd}
...
\begin{prop}
  Let $\CCat$ be a symmetric monoidal closed category. Given an idiom $(J_0, \unit, \kappa)$ on $\CCat$,
...
\begin{proof}
  For the purpose of this proof, it is easier to consider $J_0$ equivalently as a strong lax monoidal functor by \Cref{prop:idiom-corresp}.
  Then, as shown by \cite{kock-strong-functors}, $J$ is automatically a $\CCat$-enriched functor by the definition above.
  In particular, one can easily check that $J$ and $J_0$ have the same action on morphisms, i.e. for every $f: A \to B$:
  \[
  \begin{tikzcd}
    I && {[A,B]} \\
    && {[JA, JB]}
...
\begin{defn}
  Let $\CCat$ and $\DCat$ be monoidal categories and $(F, \varepsilon, \phi): \CCat \to \DCat$ be a lax monoidal functor. The \emph{change of base 2-functor}
  $F_\# : \CCat\Cat \to \DCat\Cat$ is given by applying $F$ to the $\CCat$ objects of morphisms.
\end{defn}

\begin{defn}
  For a Cartesian closed category $\CCat$ considered self-enriched, and an idiom $(J, \unit, \mult)$.
  We define the category $\Idm J = J_\#\CCat$. That is,
  \begin{mathpar}
    \ob {\Idm J} = \ob \CCat,
    \and
    \Idm J [A, B] = J[A,B].
...
\begin{prop}
  ...
  and $R$ is defined as
  \begin{mathpar}
    RA = JA, \and R_{A,B}(f) = \kappa_{A,B}(f).
  \end{mathpar}
\end{prop}
...
It is easy to define a $\CCat$-enriched functor $F: \Idm{T} \to \Kl(T)$ which is identity on objects and has its action on morphism defined as:
\[
  F_{A,B} : T[A, B] \xrightarrow{R_{A,B}} [TA, TB] \xrightarrow{[\eta_A, TB]} [A, TB]
\]
where the first arrow, $R_{A, B}$, corresponds to the \texttt{<*>} operation in the Haskell interface.
...
\begin{conj}
  Sound and complete models of this metalanguage are a CCC $\CCat$, a SMCC $\DCat$ and an identity-on-objects SMC functor $F: \CCat \to \DCat$.
\end{conj}

This is underwhelming because the models do not mention $\Idm J$.

\begin{question}
  How can we relate the above models to those that incorporate $\Idm J$?
  Is there a syntax that is sound and complete for $\Idm J$? What about $R$?
  Can we develop this further to accommodate the full ``appjuncion'' like as in CBPV?
\end{question}
...
\begin{mathpar}
  \List(\lambda x.(x,x))(\mult(\underline{m}, \underline{n})) = (\underline{m\sqcup n}, \underline{m\sqcup n}),
  \and
  \pure(x) = \List(\lambda \_.x)(\underline{\omega}),
\end{mathpar}
where $m \sqcup n = \min(m,n)$.

We can build the following program:
\[
  \cj {f:\mathbb{N} \to A} {\app{\lift{\lambda x. \app {f}{(\pi_1x)}}} {(\underline{\omega}, \underline{3})}} A
\]
...
The result will be a \emph{hom-list} of the worlds that have not been killed:
\[
  [f \mapsto f(0), f\mapsto f(1), f \mapsto f(2)]
\]
